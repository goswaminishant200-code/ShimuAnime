...
export default function AnimeDetail({ anime, episodes, characters, recs }) {
  ...
  const [seasonsLoad, setSeasonsLoad] = useState(true)
  ...
  useEffect(() => {
    if (tab === 'Comments') {
      getComments(String(anime.mal_id)).then(setComments).catch(() => {})
    }
  }, [tab, anime.mal_id])
  ...
  useEffect(() => {
    ...
    fetch(`https://api.jikan.moe/v4/anime/${anime.mal_id}/relations`)
      ...
      .then(data => {
        ...
        const seasonEntries = []
        related.forEach(r => {
          if (['Sequel', 'Prequel', 'Alternative version', 'Side story', 'Parent story'].includes(r.relation)) {
            r.entry?.forEach(e => {
              if (e.type === 'anime') {
                seasonEntries.push({ ...e, relation: r.relation })
              }
            })
          }
        })
    ...
    <main>
      ...
      <div className="relative h-[50vh] min-h-[360px]">
        <img src={image} alt={title} className="w-full h-full object-cover object-top"
          style={{ filter: 'brightness(0.22) blur(2px)' }} />
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(to top,#07070f 0%,rgba(7,7,15,0.5) 60%,transparent 100%)' }} />
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(to right,rgba(7,7,15,0.85),transparent)' }} />
      </div>
          ...
          <div className="flex-shrink-0">
            ...
            <div className="mt-4 space-y-2 w-44 md:w-52 mx-auto md:mx-0">
              ...
              <button onClick={toggleWL}
                className={`btn-ghost w-full ${inWL ? '!border-shim-primary !text-shim-accent' : ''}`}>
                <svg className="w-4 h-4" fill={inWL ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {/* Seasons Panel */}
            {!seasonsLoad && seasons.length > 0 && (
              <div className="mt-6 w-44 md:w-52 mx-auto md:mx-0">
                <p className="text-xs font-semibold text-shim-textD mb-3 uppercase tracking-wider">Related Seasons</p>
                <div className="space-y-2">
                  {/* Current */}
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-shim-primary/20 border border-shim-primary/40">
                    <div className="w-1.5 h-1.5 rounded-full bg-shim-primary flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-shim-accent truncate">Current</p>
                      <p className="text-xs text-shim-muted truncate">{title}</p>
                    </div>
                  </div>
                  {/* Related */}
                  {seasons.map(s => (
                    <Link key={s.mal_id} href={`/anime/${s.mal_id}`}
                      className="flex items-center gap-2 px-3 py-2 rounded-xl bg-shim-card border border-shim-border hover:border-shim-primary/50 hover:bg-shim-cardH transition-all group">
                      <div className="w-1.5 h-1.5 rounded-full bg-shim-muted group-hover:bg-shim-primary flex-shrink-0 transition-colors" />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-shim-textD group-hover:text-shim-accent transition-colors truncate">{s.name}</p>
                        <p className="text-[10px] text-shim-muted">{relationLabel(s.relation)}</p>
                      </div>
                      <svg className="w-3 h-3 text-shim-muted group-hover:text-shim-primary flex-shrink-0 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex-1 pt-32 md:pt-8">
            <div className="flex flex-wrap gap-2 mb-3">
              {anime.status === 'Currently Airing' && (
                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-500/20 border border-green-500/30 text-green-400 text-xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />Airing
                </span>
              )}
              {genres.slice(0, 4).map(g => (
                <Link key={g.mal_id} href={`/catalog?genre=${g.mal_id}`} className="genre-tag">{g.name}</Link>
              ))}
            </div>
            ...
            <div className="flex items-center gap-6 mb-6 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-shim-gold text-2xl">★</span>
                      ...
                      {recs.slice(0, 6).map(r => (
                        <Link key={r.entry?.mal_id} href={`/anime/${r.entry?.mal_id}`} className="group block">
                          <div className="rounded-xl overflow-hidden border border-shim-border hover:border-shim-primary/50 transition-all">
                            ...
                            <div className="p-2">
                              <p className="text-xs text-shim-textD clamp2 group-hover:text-shim-accent transition-colors">{r.entry?.title}</p>
                            </div>
                          </div>
                        </Link>
                      ))}
            ...
            {tab === 'Episodes' && (
              <div>
                {episodes.length === 0 ? (
                  <div>
                    <p className="text-shim-muted text-sm mb-4">Episode list unavailable. Manual navigation:</p>
                    <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-2">
                      {Array.from({ length: Math.min(anime.episodes || 24, 200) }, (_, i) => i + 1).map(ep => (
                        <Link key={ep} href={`/watch/${anime.mal_id}?ep=${ep}`}
                          className="flex items-center justify-center py-2.5 rounded-xl text-xs font-medium bg-shim-card border border-shim-border text-shim-textD hover:border-shim-primary hover:text-shim-accent hover:bg-shim-cardH transition-all">
                          {ep}
                        </Link>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-5">
                      {pagedEps.map(ep => (
                        <Link key={ep.mal_id} href={`/watch/${anime.mal_id}?ep=${ep.mal_id}`}
                          ...
                          <div className="w-8 h-8 rounded-lg bg-shim-bgalt flex items-center justify-center text-xs font-bold text-shim-textD group-hover:bg-shim-primary group-hover:text-white transition-all flex-shrink-0">
                            {ep.mal_id}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-shim-text group-hover:text-shim-accent transition-colors clamp2">
            ...
            {tab === 'Characters' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {characters.length === 0
                  ? <p className="text-shim-muted text-sm col-span-full">No character data.</p>
                  : characters.map(c => (
                    <div key={c.character?.mal_id} className="flex gap-3 p-3 rounded-xl bg-shim-card border border-shim-border hover:border-shim-primary/30 transition-all">
                      <img src={c.character?.images?.jpg?.image_url} alt={c.character?.name}
                        className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-shim-text clamp2 leading-snug">{c.character?.name}</p>
                        <p className="text-xs text-shim-muted mt-0.5">{c.role}</p>
                        {c.voice_actors?.[0] && (
                          <p className="text-xs text-shim-muted">VA: {c.voice_actors[0].person?.name}</p>
                        )}
                      </div>
                    </div>
                  ))
                ...
                {user ? (
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <button onClick={postComment} disabled={posting || !cmtText.trim()} className="btn-primary text-sm">
                  ...
                  <p className="text-shim-muted text-sm">
                    <Link href="/login" className="text-shim-primary hover:text-shim-accent">Login</Link> to comment.
                  </p>
                )}
